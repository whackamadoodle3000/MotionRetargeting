...
# --- Configuration ---
PICKLE_FILENAME = "res_pwalk.pk"
OUTPUT_CSV_FILENAME = "hybrik_keypoints_absolute_grounded.csv" # Changed output name
...
# Common options: 'transl', 'pred_cam_root', 'transl_camsys'. CHECK YOUR PKL FILE!
...
# 0: Pelvis, 7: L Ankle, 8: R Ankle, 20: L Wrist, 21: R Wrist
JOINT_INDICES = {
    'pelvis': 0,
    'l_foot': 7,
    'r_foot': 8,
...
def main():
    ...
    try:
        ...
        print(f"Error unpickling file '{PICKLE_FILENAME}': {e}", file=sys.stderr)
        print("The file might be corrupted, incomplete, or incompatible.", file=sys.stderr)
        sys.exit(1)
    except ModuleNotFoundError as e:
         print(f"\nError: Could not find module '{e.name}'.", file=sys.stderr)
         print("Pickle file may contain custom classes needing installation.", file=sys.stderr)
         sys.exit(1)
    ...
    if PELVIS_TRANSLATION_KEY not in data:
        ...
        print(f"Check if the key exists or try 'pred_cam_root', 'transl_camsys'.", file=sys.stderr)
        print(f"Available keys: {list(data.keys())}", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        if max_index_requested >= num_joints_total:
            print(f"Error: Requested joint index {max_index_requested} is out of bounds for {num_joints_total} joints.", file=sys.stderr)
            sys.exit(1)

        # Validate ground contact joints are requested
        ...
        print(f"Calculating absolute coordinates, adjusting ground contact, and writing to '{OUTPUT_CSV_FILENAME}'...")
        ...
        with open(OUTPUT_CSV_FILENAME, 'w', newline='') as csvfile:
            ...
            for frame_idx in range(num_frames):
                ...
                for joint_name in JOINT_ORDER:
                    ...
                    # Get the joint coordinates RELATIVE to the pelvis
                    relative_coords = relative_keypoint_data[frame_idx, joint_index, :]
                    # Calculate ABSOLUTE coordinates by adding the pelvis translation
                    # This should already be scaled correctly (hopefully in meters)
                ...
                if ADJUST_GROUND_CONTACT:
                    # Find the minimum Y among the specified ground contact joints
                    min_y_feet = float('inf')
                    for gc_joint in GROUND_CONTACT_JOINTS:
                        foot_y = frame_abs_coords[gc_joint][1] # Index 1 assumed to be Y
                        if foot_y < min_y_feet:
                            min_y_feet = foot_y
                    # Calculate shift needed to bring the lowest foot to Y=0
                    # We add this shift to all Y coordinates.
                    y_shift = -min_y_feet
                ...
                for joint_name in JOINT_ORDER:
                    ...
                    # Apply the vertical shift (if any)
                    # Create shift vector [0, y_shift, 0]
        ...
        print(f"Successfully saved absolute, grounded keypoints to '{OUTPUT_CSV_FILENAME}'.")

    except IndexError as e:
        print(f"Error accessing data: {e}. Check array dimensions and indices.", file=sys.stderr)
        sys.exit(1)
    except KeyError as e:
         print(f"Error accessing joint data, possibly a wrong JOINT_ORDER or missing ground contact joint? Details: {e}", file=sys.stderr)
         sys.exit(1)
    except Exception as e:
